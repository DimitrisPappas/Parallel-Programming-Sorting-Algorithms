...
int main(int argc, char **argv) {
  ...
  d = atoi(argv[2]); 		// Max depth of the rec tree
  N = 1<<atoi(argv[1]);
  ...
  // print();
	...
	seq_time = (double)((endwtime.tv_usec - startwtime.tv_usec)/1.0e6
  	...
  	printf("Imperative Parallel (cilk) wall clock time = %f\n", seq_time);
	...
	init();
 	 gettimeofday (&startwtime, NULL);
 	 cilk_recBitonicSort(0,N,1, 0);
 	 gettimeofday (&endwtime, NULL);

 	 seq_time = (double)((endwtime.tv_usec - startwtime.tv_usec)/1.0e6
		      + endwtime.tv_sec - startwtime.tv_sec);

 	 printf("Recursive Parallel (cilk) wall clock time = %f\n", seq_time);
